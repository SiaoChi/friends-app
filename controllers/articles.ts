...
export async function fetchUserCreateArticle(req: Request, res: Response) {
  try {
    ...
    const result = await articleModels.createUserArticle(
      title,
      content,
      date,
      userId
...
export async function renderArticleByID(req: Request, res: Response) {
  try {
    ...
    const {userId} = res.locals;
    const singleArticleData = await articleModels.getArticleByID(
      parseInt(articleId)
    );
    const userArticleEmojiId = await articleModels.getArticleEmojiId(
      parseInt(articleId),
      userId
    );

    if (
      Array.isArray(singleArticleData) &&
      singleArticleData.length > 0 &&
      userArticleEmojiId.length > 0
    ) {
      return res
        .status(200)
        .render("singleArticle", {singleArticleData, userArticleEmojiId});
    } else if (
      Array.isArray(singleArticleData) &&
      singleArticleData.length > 0 &&
      userArticleEmojiId.length === 0
      ...
      return res
        .status(200)
        .render("singleArticle", {singleArticleData, userArticleEmojiId: null});
    }
...
export async function updateArticleById(req: Request, res: Response) {
  try {
    ...
    const singleArticleData = await articleModels.updateArticleByID(
      id,
      title,
      content,
      date
...
export async function saveArticleEmoji(req: Request, res: Response) {
  try {
    ...
    const affectRows = await articleModels.saveArticleEmoji(
      userId,
      parseInt(articleId),
      parseInt(emojiId)
    );
