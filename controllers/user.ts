...
export async function signIn(req: Request, res: Response) {
  try {
    ...
    const isValidPassword = await userLoginModel.checkNativeProviderToken(
      user.id,
      password
    );
    if (!isValidPassword) {
...
export async function renderUserProfile(req: Request, res: Response) {
  const userId = res.locals.userId;
  const userProfile = (await userModel.getUserProfileTagsData(
    userId
  )) as RowDataPacket[];
  const userArticles = (await articleModel.getUserArticlesData(
    userId
  )) as RowDataPacket[];
