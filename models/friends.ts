...
export async function getUserTagsIdByUserId(userId: number) {
  const [results] = await pool.query(
    ` SELECT tag_id FROM users_tags WHERE user_id = (?)`,
    [userId]
  );
...
export async function getUsersIdByTagId(userId: number, userTagsId: number[]) {
  const [results] = await pool.query(
    `
        SELECT user_id FROM users_tags WHERE tag_id IN (?) AND user_id != ? `,
    [userTagsId, userId]
  );
  ...
  /*
    ...
    =>  { '4': 2, '6': 1, '15': 1 }
     */
  const mapFriends = tagOfUsers.reduce(
    (acc: {[userId: number]: number}, item) => {
...
export async function filterIgnoreFriends(
  userId: number,
  mapFriends: {[key: number]: number}
) {
